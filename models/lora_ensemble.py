...
class LoRAEnsemble(nn.Module):
    def __init__(
        ...
        #     self.lora_layers = list(range(len(self.model.transformer.decoder.layers)))
        # else:
        #     self.lora_layers = lora_layers

        # Freeze Vision GroundingDINO weights
        for param in self.model.parameters():
            param.requires_grad = False
        
        dec_layer = self.model.transformer.decoder.layers[5].ca_text

        # Extract dimensions for the projections of the layer
        for char in lora_type:
            if char != "q":
                dim = getattr(dec_layer.cross_attention, f"{char}dim")
            else:
                dim = dec_layer.cross_attention.embed_dim

            # Replace the original projection layers with the LoRA layers
            setattr(dec_layer.cross_attention, f"{char}_proj",
                    EnsembleLoRA(
                        getattr(dec_layer.cross_attention, f"{char}_proj"),
                        rank=rank,
                        dim=dim,
                        n_members=n_members,
                        initialize=True,
        ...
        dim = dec_layer.cross_attention.embed_dim
        setattr(dec_layer.cross_attention, "out_proj",
                EnsembleLoRA(
                    getattr(dec_layer.cross_attention, "out_proj"),
                    rank=rank,
                    dim=dim,
                    n_members=n_members,
                    initialize=True,
        ...
        # for layer_id, dec_layer in enumerate(self.model.transformer.decoder.layers[5].ca_text):
        ...
        #     for char in lora_type:
        #         if char != "q":
        #             dim = getattr(dec_layer.self_attention, f"{char}dim")
        #         else:
        #             dim = dec_layer.self_attention.embed_dim

        #         # Replace the original projection layers with the LoRA layers
        #         setattr(dec_layer.self_attention, f"{char}_proj",
        #                 EnsembleLoRA(
        #                     getattr(dec_layer.self_attention, f"{char}_proj"),
        #                     rank=rank,
        #                     dim=dim,
        #                     n_members=n_members,
        #                     initialize=True,
        ...
        #                 )
        #     dim = dec_layer.self_attention.embed_dim
        #     setattr(dec_layer.self_attention, "out_proj",
        #             EnsembleLoRA(
        #                 getattr(dec_layer.self_attention, "out_proj"),
        #                 rank=rank,
        #                 dim=dim,
        #                 n_members=n_members,
        #                 initialize=True,
    ...
    def gather_params(self):
        ...
        enc_layer = self.model.transformer.decoder.layers[5].ca_text
        for char in self.lora_type:
            # Gather the parameters of the LoRA module
            proj_params = enc_layer.cross_attention.__getattr__(f"{char}_proj").params
            proj_params = {f"decoder_layer_5_{char}_proj_{k}": v for k, v in proj_params.items()
                            if k in ["w_a.weight", "w_b.weight"]}
            params.update(proj_params)

        # Gather the parameters of the out projection
        out_proj_params = enc_layer.cross_attention.out_proj.params
        out_proj_params = {f"decoder_layer_5_out_proj_{k}": v for k, v in out_proj_params.items()
                            if k in ["w_a.weight", "w_b.weight"]}
